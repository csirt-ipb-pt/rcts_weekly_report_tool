...
fileToSearch = (str(path) + '/upload/other.csv') # Tuple that joins the directory path where the script is located and the subdirectories and name of the .csv file.
print(fileToSearch)
...
def import_to_db(listdic, persistent):
    for i in range(0, len(listdic)):
        ...
        if sclice[0] in persistent:
            ...
            Other.objects.filter(ip=sclice[0]).update(rede= sclice[1])
            if sclice[2] <= data[1]:
                Other.objects.filter(ip=sclice[0]).update(data_1= sclice[2])
            if sclice[3] >= data[2]:
                Other.objects.filter(ip=sclice[0]).update(data_2= sclice[3])
            val = int(data[3])
            value = val + sclice[4]
            Other.objects.filter(ip=sclice[0]).update(count= value)
        else:
            new_other = Other(ip=sclice[0], rede=sclice[1], data_1=sclice[2], data_2=sclice[3], count=sclice[4])
            new_other.save()
